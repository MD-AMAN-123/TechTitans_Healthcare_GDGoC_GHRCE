...
const Appointments: React.FC<AppointmentsProps> = ({ 
  ...
  const handleConfirmBooking = () => {
    ...
    const meetCode = `${Math.random().toString(36).substring(7)}-${Math.random().toString(36).substring(7)}-${Math.random().toString(36).substring(7)}`;
           ...
           {dates.map((date) => {
             ...
             return (
               <button
                 ...
                 className={`flex-shrink-0 min-w-[72px] p-3 rounded-2xl border flex flex-col items-center gap-1 transition-all duration-200 ${
                   isSelected 
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md animate-fade-in">
           <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[32px] p-8 text-center relative overflow-hidden shadow-2xl">
              ...
              <p className="text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
                Your appointment with <span className="font-bold text-slate-700 dark:text-slate-300">{selectedSpecialist.name}</span> has been successfully scheduled.
              </p>
