...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
      <div className="bg-white rounded-[32px] shadow-2xl p-8 w-full max-w-md text-center animate-fade-in border border-slate-100">
         {/* Attractive Logo */}
         <div className="flex flex-col items-center justify-center mb-8">
            ...
            <h1 className="text-4xl font-bold tracking-tight">
              <span className="text-[#1e3a8a]">Medi</span>
         ...
         {error && !isAdminModalOpen && !isGuestModalOpen && (
            <div className="mb-6 p-3 bg-red-50 border border-red-100 rounded-xl text-red-600 text-xs text-left flex gap-2 items-start">
         ...
         <div className="space-y-3">
           <button 
             ...
             disabled={isLoading}
             className="w-full bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-center gap-3 hover:bg-slate-50 hover:border-slate-300 transition-all group relative overflow-hidden shadow-sm hover:shadow-md disabled:opacity-70 disabled:cursor-not-allowed"
           >
             {isLoading ? (
           ...
           <div className="relative flex items-center justify-center my-4">
             ...
             <span className="bg-slate-50 px-3 text-xs text-slate-400 relative z-10 font-bold tracking-wide">OR</span>
           </div>

           <button 
             onClick={handleGuestLoginClick}
             disabled={isLoading}
             className="w-full bg-[#1e3a8a] text-white border border-transparent rounded-2xl p-4 flex items-center justify-center gap-3 hover:bg-blue-900 transition-all shadow-lg shadow-blue-900/20 disabled:opacity-70 disabled:cursor-not-allowed"
           >
             {isLoading ? (
           ...
           <button 
             ...
             disabled={isLoading}
             className="w-full mt-2 bg-slate-50 text-slate-400 border border-transparent rounded-2xl p-3 flex items-center justify-center gap-2 hover:bg-slate-100 hover:text-slate-600 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
           >
              {isLoading ? (
      ...
      {isGuestModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
           <div className="bg-white w-full max-w-sm rounded-[32px] p-8 relative overflow-hidden shadow-2xl">
              <button 
              ...
              <div className="flex flex-col items-center text-center mb-6">
                 ...
                 <h3 className="text-xl font-bold text-slate-800">Guest Details</h3>
                 <p className="text-slate-500 text-sm mt-1">Please provide your details to continue.</p>
              </div>

              <form onSubmit={submitGuestLogin} className="space-y-4">
                 <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 ml-1">Name (Optional)</label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-semibold focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all text-slate-800"
                        autoFocus
                      />
                    </div>
                 </div>
                 ...
                 <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 ml-1">Mobile Number (Mandatory)</label>
                    <div className="relative">
                 ...
                 {error && (
                    <div className="text-xs text-red-500 font-bold bg-red-50 p-2 rounded-lg text-center flex items-center justify-center gap-1">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
           <div className="bg-white w-full max-w-sm rounded-[32px] p-8 relative overflow-hidden shadow-2xl">
              ...
              <div className="flex flex-col items-center text-center mb-6">
                 ...
                 <p className="text-slate-500 text-sm mt-1">Enter your administrator credentials.</p>
              </div>
                 ...
                 <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        placeholder="••••••••"
                        className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-800"
                        autoFocus
                      />
                    </div>
                 </div>
