                ...
                <div className="flex flex-col gap-2">
                    <div className="bg-white/5 backdrop-blur-md rounded-xl p-3 border border-white/10 flex items-center gap-3 transition-transform hover:scale-102">
                       <div className={`p-2 rounded-lg ${
                            healthStatus === 'Excellent' || healthStatus === 'Good' ? 'bg-emerald-500/20 text-emerald-400' : 
                       ...
                       <div>
                          ...
                          <p className={`text-sm font-bold ${
                              healthStatus === 'Excellent' || healthStatus === 'Good' ? 'text-emerald-400' : 
              ...
              {aiInsights.length > 0 ? aiInsights.map((tip, idx) => (
                <div key={idx} className="flex gap-3 items-start animate-fade-in" style={{ animationDelay: `${idx * 100}ms` }}>
                   <div className={`p-2.5 rounded-xl flex-shrink-0 ${
                     tip.category === 'Nutrition' ? 'bg-emerald-50 dark:bg-emerald-900/20' :
      ...
      <div className="bg-teal-50 dark:bg-teal-900/10 rounded-3xl p-5 border border-teal-100 dark:border-teal-900/30">
        ...
        <div className={`space-y-3 ${showAllAppointments ? 'flex flex-col' : ''}`}>
           {(displayAppointments).map((apt) => {
              ...
              // 4. Time is "near" (<= 30 mins before) or "active" (> -60 mins after start)
              if (isToday && apt.meetLink && apt.status !== 'cancelled' && apt.status !== 'completed') {
                 ...
                 // Difference in minutes (aptTime - now)
                 // Positive = Future (e.g. starts in 10 mins) -> diff > 0
                 // Negative = Past (e.g. started 10 mins ago) -> diff < 0
                 // Formula: Appointment Time - Current Time
                 const diff = (aptTime.getTime() - now.getTime()) / 60000;
                 
                 // Logic: 
                 // If diff is 30 (30 mins BEFORE start) -> Show
                 // If diff is -60 (60 mins AFTER start) -> Show
                 // So range is: diff <= 30 AND diff > -60
                 if (diff <= 30 && diff > -60) {
                    showJoinButton = true;
                 }
              }
                ...
                {showJoinButton && (
                  <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end animate-fade-in">
                    <a 
                      ...
                      className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-500/20 hover:scale-105 animate-pulse"
                    >
                      <Video size={18} />
                      Join Google Meet
                    </a>
                   ...
                   <>
                    ...
                    <div className={`absolute inset-0 transition-opacity flex items-center justify-center gap-2 ${isPremium ? 'bg-black/30 opacity-0 group-hover:opacity-100' : 'bg-black/10'}`}>
                      ...
                      <button 
                        ...
                        className={`backdrop-blur-md p-2.5 rounded-full transition-all border ${
                          isPremium 
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in" onClick={() => setActiveNotification(null)}>
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-6 shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col items-center text-center">
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-5 shadow-lg ${
                        activeNotification.type === 'alert' ? 'bg-red-100 text-red-600 shadow-red-500/20' :
